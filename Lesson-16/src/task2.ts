...
if (confirm('Почати тестування?')) {
    ...
    class Firm {
        ...
        private _currentYear = 2025
        private _name = ''
        private _dateFoundation: DateFoundation = {
            year: 0,
            month: 0
        }
        private _services: Service[] = []
        private _branchAddresses: BranchAddress[] = []

        constructor(initName: string, initDateFoundation: DateFoundation, initServices: Service[], initBranchAddresses: BranchAddress[]) {
            this.Name = initName
        ...
        private get DateFoundation(): DateFoundation {
            return this._dateFoundation
        }
        private get Services(): Service[] {
            return this._services
        }
        private get BranchAddresses(): BranchAddress[] {
            return this._branchAddresses
        }

        private set Name(name: string) {
            if (name.length === 0) throw new Error("Error: Name is empty!");
            this._name = name;
        }
        private set DateFoundation(dateObj: DateFoundation) {
            if (Object.keys(dateObj).length === 0) throw new Error("Error: Date foundation object is empty!");

            if (dateObj.year <= this.CurrentYear) {
                this._dateFoundation = JSON.parse(JSON.stringify(dateObj)) as DateFoundation
            } else {
                throw new Error("Error: Year foundation does not be greater than current year!");
            }
        }
        private set Services(dataList: Service[]) {
            if (dataList.length === 0) throw new Error("Error: Services list is empty!");
            this._services = JSON.parse(JSON.stringify(dataList)) as Service[]
        }
        private set BranchAddresses(dataList: BranchAddress[]) {
            if (dataList.length === 0) throw new Error("Error: Branch addresses list is empty!");
            this._branchAddresses = JSON.parse(JSON.stringify(dataList)) as BranchAddress[]
        }

        private printName(name: string) {
            document.write(`<h1>Name firm:${name}</h1>`)
        }
        private printDateFoundation(dateFoundationObj: DateFoundation) {
        ...
        private printServices(servicesList: Service[]) {
        ...
        private printBranchAddresses(branchAddressesList: BranchAddress[]) {
         ...
         */
        public get numberYearsFirmExistence(): number {
         ...
         */
        public displayBranchesListByCity(city: string) {
            if (city.length === 0) throw new Error("Error: City is empty!");
         ...
         */
        public displayServicesAvailableWithinBudgetAndTime(amountMoney: number, daysCompletion: number) {
            if (amountMoney < 1 || daysCompletion < 1) throw new Error("Error: Input data is incorrect!")
    ...
    const dateFoundation: DateFoundation = {
        year: 1994,
        month: 5
    }
    ...
    const branchAddresses: BranchAddress[] = [
        {
            country: "USA",
            city: "New York",
            street: "5th Avenue",
    ...
    function printValue(value: number, msg: string) {
