 ...
 */
function removeIfExs(selector: string) {
...
function printRes(res: number) {
    ...
    if (document.querySelector('.trip')) {
        ...
        (document.querySelector('.trip') as HTMLInputElement).append(result)
    }
}
...
*/
function getSumFromList(someList: any): number {
    let resSum = 0
    for (const item of someList) {
        if ((item as HTMLInputElement).checked) {
            resSum += parseFloat((item as HTMLInputElement).value)
        }
    }
...
function getTotalCost() {
    ...
    const transportationCost = parseFloat((document.querySelector('.type-transport') as HTMLInputElement).value);
...
window.onload = function () {
    ...
    if (document.getElementById('trip-btn')) {
        (document.getElementById('trip-btn') as HTMLInputElement).onclick = getTotalCost
    }
}
