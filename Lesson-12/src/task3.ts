...
const arrayNumbers: number[] = Array.from({ length: 30 }, () => Math.floor(Math.random() * 100))
...
for (let k = 1; k < arrayNumbers.length; k++) {
    const currentElement: number = arrayNumbers[k]
    let i: number = k - 1
...
function printTable(arr: number[], label: string = "") {
    ...
    if (label) document.write(`<p><i>${label}</i></p>`)
}
