...
function Partition(A: string[], p: number, q: number): number {
    const pivot: string = A[q];
    let i: number = p - 1;
    for (let j = p; j < q; j++) {
        if (A[j] <= pivot) {
            i = i + 1;
...
function Quicksort(A: string[], p: number, q: number): void {
    if (p >= q) return;
    const i: number = Partition(A, p, q);
    Quicksort(A, p, i - 1);
    Quicksort(A, i + 1, q);
}
...
function includes_binarySearch(arr: string[], searchElement: string): number {
    let start: number = 0
